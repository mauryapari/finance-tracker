...
const ButtonStub = {
  props: ['label', 'severity', 'size', 'variant'],
  template: '<button @click="$emit(\'click\')">{{ label }}</button>',
  emits: ['click'],
}

const mountSavePopup = (props = {}) =>
  mount(SavePopup, { props, global: { stubs: { Button: ButtonStub } } })

describe('SavePopup — save mode (default)', () => {
  it('shows Save and Cancel buttons', () => {
    const w = mountSavePopup()
...
describe('SavePopup — delete mode', () => {
  it('shows "Delete this row?" text', () => {
    const w = mountSavePopup({ mode: 'delete' })
